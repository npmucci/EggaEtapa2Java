...
/*1- Obtener los datos completos de los empleados*/
 ...
 SELECT * FROM departamentos;
  ...
  /*3- Listar los nombres de los departamentos*/
 ...
 /*9- Obtener el nombre de o de los jefes que tengan su departamento situado en la ciudad de "Ciudada Real*/
 SELECT nombre_jefe_depto, ciudad FROM departamentos WHERE ciudad = "Ciudad Real";
 
 /*10-Elabora un listado donde para cada fila, figure el alias "nombre" y "cargo" para las respectivas tablas de los empleados*/
 SELECT nombre AS "Nombre", cargo_emp As "Cargo" FROM empleados;
 
 /*11- Listar los salarios y comisiones de los empleados del departamentos 2000, ordenado por comision de menor a mayor*/
 
 SELECT sal_emp As Salario, comision_emp AS Comision, id_depto AS Departamento FROM empleados WHERE id_depto=2000 ORDER BY comision_emp  ASC;
 
 /* 12- Obtener el valor total a pagar a cada empleado del departamento 3000, que resulta de: sumar el salario
 mas una bonificacion de 500. Mostrar el nombre del empleado y el total a pagar en order alfabético*/
 
 SELECT nombre AS "Nombre", (sal_emp + comision_emp + 500) AS "Total a Pagar" FROM empleados ORDER BY nombre ASC;
...
SELECT sal_emp AS Salario, comision_emp AS Comision, (sal_emp + comision_emp) AS 'Salario Total'FROM empleados WHERE comision_emp > 1000;

/*15. Obtener un listado similar al anterior, pero de aquellos empleados que NO tienen
comisión.*/

SELECT sal_emp AS Salario, comision_emp AS Comision, (sal_emp + comision_emp) AS 'Salario Total', nombre AS Nombre
...
SELECT nombre As Nombre,sal_emp As Salrio, (sal_emp*0.30) As '30 % Salario', comision_emp As Comisión FROM empleados WHERE comision_emp <= (sal_emp * 0.30);
...
SELECT MAX(sal_emp) AS 'Salario Maximo', MIN(sal_emp) AS 'Salario Minimo', MAX(sal_emp) - MIN(sal_emp) AS 'Diferencia Salarios'
FROM empleados;
...
SELECT COUNT(id_emp) AS NumeroEmpleados, id_depto FROM empleados GROUP BY id_depto HAVING COUNT(id_emp) > 3;

/*26. Hallar los departamentos que no tienen empleados*/
SELECT nombre_depto AS NombreDepartamento
FROM departamentos 
LEFT JOIN empleados ON departamentos.id_depto = empleados.id_depto
WHERE empleados.id_emp IS NULL;
