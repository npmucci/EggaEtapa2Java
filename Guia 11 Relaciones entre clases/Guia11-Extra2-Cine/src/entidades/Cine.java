...
public class Cine {
        ...
        public Cine(Pelicula pelicula, int filas, int columnas) {
            this.pelicula = pelicula;
            this.sala = new String[filas][columnas];
            this.random = new Random();

            // Inicializar la sala con asientos vacíos
        ...
        public void ubicarPersonas(Espectador persona, Pelicula pelicula) {
            if (persona.getEdad() >= pelicula.getEdadMinima() && persona.getDineroDisponible() >= pelicula.getPrecioEntrada()) {
                ...
                while (!sala[filaAleatoria][columnaAleatoria].equals(" ")) {
                    filaAleatoria = random.nextInt(filas);
                    columnaAleatoria = random.nextInt(columnas);
                }

                sala[filaAleatoria][columnaAleatoria] = "X";
            }
        }
    ...
    public void mostrarSala() {
        ...
        for (int i = 0; i < filas; i++) {
            int numeroFila = filas - i;
            for (int j = 0; j < columnas; j++) {
                System.out.print(numeroFila + " " + (char) ('A' + j) + " " + sala[i][j] + " | ");
            }
            System.out.println();
        }
    }
